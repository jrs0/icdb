...
hbr_data <- data.frame(
    ...
    sex = sample(factor(c("male","female")), N, replace=1, prob=c(config[["pct_male"]],1-config[["pct_male"]])),
    ## MAJOR criteria - from figure 1
    major_anaemia = sample(c(1,0), N, replace=1, prob=c(config[["pct_sev_anaemia"]],1-config[["pct_sev_anaemia"]])),
    major_oac = sample(c(1,0), N, replace=1, prob=c(config[["pct_oac"]],1-config[["pct_oac"]])),
    major_malig = sample(c(1,0), N, replace=1, prob=c(config[["pct_malig"]],1-config[["pct_malig"]])),
    major_sev_ckd = sample(c(1,0), N, replace=1, prob=c(config[["pct_sev_ckd"]],1-config[["pct_sev_ckd"]])),
    major_surgery = sample(c(1,0), N, replace=1, prob=c(config[["pct_surgery"]],1-config[["pct_surgery"]])),
    major_thrmcyt = sample(c(1,0), N, replace=1, prob=c(config[["pct_thrmcyt"]],1-config[["pct_thrmcyt"]])),
    ## MINOR criteria - from figure 1
    minor_age_gt75 = if_else(rnorm(N, mean=config[["age_mean"]], sd=config[["age_sd"]])>=75, 1, 0),
    minor_mod_ckd = sample(c(1,0), N, replace=1, prob=c(config[["pct_mod_ckd"]],1-config[["pct_mod_ckd"]])),
    minor_mild_an = sample(c(1,0), N, replace=1, prob=c(config[["pct_mild_anaemia"]],1-config[["pct_mild_anaemia"]])),
    major_cva = sample(c(1,0), N, replace=1, prob=c(config[["pct_cva"]],1-config[["pct_cva"]])),
    minor_bleed = sample(c(1,0), N, replace=1, prob=c(config[["pct_bleed"]],1-config[["pct_bleed"]]))
) %>%
...
## Check how well we did with ARC score proportions
hbr_data %>%
    mutate(n=n()) %>%
    group_by(arc_hbr_score) %>%
