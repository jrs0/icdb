...
srv$APC_SYNTH %>% filter(SpellStartDate > !!ymd("2019-1-1")) %>% select(NHSNumber, PrimaryDiagnosis_ICD) %>% show_query()
```
...
srv$APC_SYNTH %>% filter(SpellStartDate > !!ymd("2019-1-1")) %>% select(NHSNumber, PrimaryDiagnosis_ICD) %>% run()
```
...
```{r}
results <- srv$APC_SYNTH %>% filter(SpellStartDate > !!ymd("2019-1-1")) %>% select(NHSNumber, PrimaryDiagnosis_ICD) %>% run()
```
